...
class AppTest {
  ...
  constructor() {
    ...
    this.user = new UserData(this.root);
  ...
  async testUser() {
    ...
    await this.user.editUser( key = '', { name: 'Kikz Lagazon', email: 'francislagazon@gmail.com' })
      .then(x => this.error('User edit with empty id must be failed'))
      .catch(e => {
        if (e.message == ERROR.user_id_empty) this.success("User with empty id failed properly.");
        else this.error(`Something wrong with User edit. It should be failed with User id empty error: ${e.message}`);
      });

    // Check if the user is existing //
    await this.user.editUser( key = 'user3', { name: 'Kikz Lagazon', email: 'francislagazon@gmail.com' })
      .then(x => this.error('User edit with empty id must be failed'))
      .catch(e => {
        if (e.message == ERROR.user_not_exist) this.success(`User ${key} is not exist.`);
        else this.error(`Something wrong with User edit. It should be failed with User id empty error: ${e.message}`);
      });

    // Check to update user //
    await this.user.editUser( key = 'user1', { name: 'Kikz Lagazon', email: 'francislagazon@gmail.com' })
      .then(x => this.success(`Successfuly update with key: ${key}`))
      .catch(e => {
        this.error(`Failed to update user: ${key} with error: ${e.message}`);
      });

    // Check to update user with empty data //
    await this.user.editUser( key = 'user1', { })
      .then(x => this.error(`User data is empty with key: ${key}`))
      .catch(e => {
        if (e.message == ERROR.user_data_empty) this.success(`User data is empty.`);
        else this.error(`Something wrong with User edit. It should be failed with User data empty error: ${e.message}`);
      });
